...
document.getElementById('startBtn').addEventListener('click', async () => {
    ...
    // Check last key generation time
    const lastGeneration = localStorage.getItem('lastKeyGeneration');
    const currentTime = Date.now();
    if (lastGeneration && currentTime - lastGeneration < 3600000) { // 3600000 ms = 1 hour
        alert('You can only generate keys once per hour.');
    ...
    // Update last key generation time
    localStorage.setItem('lastKeyGeneration', Date.now());
